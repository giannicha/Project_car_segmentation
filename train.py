...
def main(config_):
    ...
    for batch_idx, batch_size in enumerate(loader_dict['batch_size']):
        ...
        # 배치사이즈에 따른 데이터 로더 생성
        train_dataloader = make_dataloder(data_dir=loader_dict['data_dir'], transform=train_transform,
                                          train_=True, batch_size=batch_size)

        valid_dataloader = make_dataloder(data_dir=loader_dict['data_dir'], transform=test_transform,
        ...
        for lr_idx, learning_rate in enumerate(config.optimizer['args']['lr']):
            ...
            for ep_idx, epoch in enumerate(config.trainer['epochs']):
                ...
                logger = Logger(config_file=train_config, p_name=config.module_name,
                                r_name=f'ver{batch_idx + lr_idx + ep_idx}')
                ...
                trainer.train()
                config = ConfigParser(config=config_, run_id=f'ver{batch_idx + lr_idx + ep_idx}')
